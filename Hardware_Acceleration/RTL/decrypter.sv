...
`define CHECK_VAL 5'b11000
`define RESTARTING 5'b11001
`define CONCLUDED 5'b11010
...
module decrypter(
    ...
    always_ff @(posedge clk) begin
        if (reset) begin
            state <= `IDLE;
        end else if (found && !finished) begin
            state <= `CONCLUDED;
        end else begin
            case(state)
                ...
                `LOAD_RAM: state <= `CHECK_VAL;
                
                `CHECK_VAL: begin
                    if ((temp_decrypt == 8'd32) | ((temp_decrypt >= 8'd97) && (temp_decrypt <= 8'd122))) begin
                        state <= `CHECK;
                    end else begin
                        state <= `RESTARTING;
                    end
